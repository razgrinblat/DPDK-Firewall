...
public:
    HttpDpiModule(const HttpDpiModule&) = delete;
    HttpDpiModule& operator=(const HttpDpiModule&) = delete;
    static HttpDpiModule& getInstance();

    void onHttpMessageCallBack(const pcpp::TcpStreamData &tcpData);
